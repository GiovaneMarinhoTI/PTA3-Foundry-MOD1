...
export default class MoveImporter extends PtaApplication {
    ...
    static async _onSubmit(event, target) {
        ...
        for (const move of this.move_selections) {
            ...
            const customAttackData = await utils.fetchCustomAttackData(move);
            console.log(`[v0] move-importer: customAttackData for ${move}:`, customAttackData);
            if (customAttackData) {
                data.range = customAttackData.range;
                data.frequency = customAttackData.frequency;
                console.log(`[v0] move-importer: Applied range=${data.range}, frequency=${data.frequency}`);
                // If custom API has a description, use it (it may be more detailed)
                if (customAttackData.description) {
                    data.description = customAttackData.description;
                }
            }
