...
export default class PtaPokemonSheet extends PtaActorSheet {
    static DEFAULT_OPTIONS = {
        window: {
            controls: [{
                icon: 'fas fa-dice-d6',
                label: 'PTA.Button.GenerateSprite',
        ...
        actions: {
            generateSprite: this._onGenerateSprite,
    ...
    static get PARTS() {
        ...
        p.passives = { template: `${this.TEMPLATE_PATH}/actor/pokemon/passives.hbs` };
    ...
    static TABS = {
        ...
        passives: { id: "passives", group: "primary", label: "PTA.Tab.Passives", icon: "fa-bolt" },
    }
    ...
    static _normalizePokemonName(name) {
        if (!name) return "";
        ...
        normalized = normalized.replace(/(?:\b)(hisui|hisuian|galar|galarian|alola|alolan|paldea|paldean)(?:\b)/g, v => aliases[v] ?? v);
        return normalized;
    }

    static _parsePokemonVariant(raw) {
        const query = this._normalizePokemonName(raw);
        if (!query) return { query: '', base: '', region: null, form: null };
        ...
        if (regionMap[lastPart]) {
            return {
                query,
                base: parts.slice(0, -1).join('-'),
                region: regionMap[lastPart],
                form: null
            };
        }
        return { query, base: query, region: null, form: null };
    }

    static async _findSpriteForActor(actor) {
        const module = game.modules.get("pokemon-assets");
        if (!module?.active) return null;
        const api = module.api;
        if (!api?.PokemonSheets) return null;

        const speciesRaw = actor.system.species || actor.name || "";
        let parsed = this._parsePokemonVariant(speciesRaw);
        if (!parsed.region && actor.name && actor.name !== actor.system.species) {
            parsed = this._parsePokemonVariant(actor.name);
        }

        let pokemon = PTA.Pokedex.getPokemon(parsed.query);
        if (!pokemon && parsed.base !== parsed.query) {
            pokemon = PTA.Pokedex.getPokemon(parsed.base);
        }
        if (!pokemon) return null;

        const gender = (actor.system.gender || "").toLowerCase();
        const shiny = Boolean(actor.system.shiny);
        return api.PokemonSheets.getPokemon({ dex: pokemon.id, shiny, gender, region: parsed.region, form: parsed.form });
    }

    static async _onGenerateSprite() {
        const module = game.modules.get("pokemon-assets");
        ...
        const result = await PtaPokemonSheet._findSpriteForActor(this.document);
        if (!result?.img) return ui.notifications.warn(game.i18n.localize("PTA.Error.PokemonSpriteNotFound") || "No matching Pokemon Assets sprite was found for this Pokémon.");

        const updateData = { img: result.img };
        if (result.settings) updateData.prototypeToken = result.settings;
        await this.document.update(updateData);

        ui.notifications.info(game.i18n.localize("PTA.Button.GenerateSprite"));
    }
