...
function registerHelpers() {
    const helpers = {
        ...
        ,
        // Return translated skill label (case-insensitive map + i18n fallback)
        skillLabel: (key) => {
            try {
                if (!key) return '';
                const skillMap = {
                    ...
                    'stealth': 'Furtividade',
                    'subterfuge': 'Prestidigitação'
                };
                const raw = String(key || '');
                const lower = raw.toLowerCase();
                if (skillMap[lower]) return skillMap[lower];

                // Try i18n keys with TitleCase and lowercase variants
                const toTitle = s => s.replace(/(^|[ _-])(\w)/g, (_, __, c) => c.toUpperCase()).replace(/[ _-]/g, '');
                const variants = [raw, raw.charAt(0).toUpperCase() + raw.slice(1), toTitle(raw), lower];
                for (const v of variants) {
                    try {
                        const localized = game.i18n.localize(`PTA.Skill.${v}.long`);
                        if (localized && !/^[A-Za-z0-9_.:-]+$/.test(localized)) return localized;
                    } catch (e) { /* ignore */ }
                }

                // Final fallback: return key in Title Case
